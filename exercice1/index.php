 ...
 * Je sélectionne les colonnes qui contient le nom et prénom de la table clients par ordre alphabétique
 * Et je demande de récupérer chaque ligne de ceci
 */
$answer = $database->query('SELECT lastName, firstName FROM clients ORDER BY lastName');
...
<html>
    ...
    <body>
        ...
        <h1 class="text-center">La liste des clients</h1><hr/>
        <div class="container">
            <div class="row">     
                <?php
                 /* J'affiche tous les noms et prénom des données récupérer */
                foreach ($data as $clients) {
                    ?>
                    <p class="col-lg-2">
                        <?php
                        echo $clients['lastName'] . ' ' . $clients['firstName'];
                        ?>
                    </p>
                    <?php
                }  
